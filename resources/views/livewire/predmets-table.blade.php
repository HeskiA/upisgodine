            ...
            <table class="w-full border">
                <thead>
                    <tr>
                        ...
                        <th class="border px-1 py-2">Naziv</th>
                        <th class="border px-4 py-2">Modul</th>
                        <th class="border px-1 py-2">Kapacitet</th>
                        <th class="border px-1 py-2">Popunjeno</th>
                        <th class="border px-4 py-2">Stvoren</th>
                        <th class="border px-2 py-2">Ažuriran</th>
                ...
                <tbody>
                    @foreach($predmets as $predmet)
                        <tr>
                            ...
                            <td class="border px-1 py-2 text-center">{{ $predmet->naziv }}</td>
                            @php
                                $modul = App\Http\Livewire\PredmetsTable::getModul($predmet); 
                            @endphp
                            <td class="border px-4 py-2 text-center">{{ $modul }}</td>
                            <td class="border px-1 py-2 text-center">{{ $predmet->kapacitet }}</td>
                            <td class="border px-1 py-2 text-center">{{ $predmet->popunjeno }}</td>
                            <td class="border px-4 py-2 text-center">{{ $predmet->created_at }}</td>
                            <td class="border px-1 py-2 text-center">{{ $predmet->updated_at }}</td>
                            <td class="border px-4 py-2 text-center">
                                <button wire:click="otvoriUpdateModal({{ $predmet->id }})" class="w-1/2 bg-green-600 hover:bg-green-700 text-white font-bold py-1 px-2 my-1 rounded">Uredi</button>
                                <button wire:click="delete({{ $predmet->id }})" class="w-1/2 bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded">Izbriši</button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
